...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (currentEmployee.role === "admin" && role !== "admin") {
    const admins = await d1.prepare("SELECT COUNT(*) AS total FROM users WHERE company_id = ? AND role = 'admin'").bind(access.user!.companyId).first<{ total: number }>();
    if ((admins?.total ?? 0) <= 1) return Response.json({ error: "O sistema precisa manter pelo menos um administrador." }, { status: 400 });
  }
  await d1.prepare("UPDATE users SET name = ?, role = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND company_id = ?").bind(name, role, employeeId, access.user!.companyId).run();
  const employee = await d1.prepare("SELECT id, name, email, role, email_verified_at AS emailVerifiedAt, created_at AS createdAt FROM users WHERE id = ? AND company_id = ?").bind(employeeId, access.user!.companyId).first();
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const d1 = await getD1();
  const employee = await d1.prepare("SELECT id, role FROM users WHERE id = ? AND company_id = ?").bind(employeeId, access.user.companyId).first<{ id: number; role: string }>();
  if (!employee) return Response.json({ error: "Funcionário não encontrado." }, { status: 404 });
  if (employee.role === "admin") {
    const admins = await d1.prepare("SELECT COUNT(*) AS total FROM users WHERE company_id = ? AND role = 'admin'").bind(access.user.companyId).first<{ total: number }>();
    if ((admins?.total ?? 0) <= 1) return Response.json({ error: "O sistema precisa manter pelo menos um administrador." }, { status: 400 });
  }
  await d1.prepare("DELETE FROM users WHERE id = ? AND company_id = ?").bind(employeeId, access.user.companyId).run();
